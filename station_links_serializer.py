...
class StationLinksSerializer:
    def serialize(self, station_links: station_links.StationLinks, file_name):
        with open(file_name, 'w', encoding='iso-8859-1', newline='\r\n') as f:
            self.serialize_(station_links, f)
    
    def serialize_(self, station_links: station_links.StationLinks, f):
        print("---------------------------", file=f)
        print("Time Table StnLinkList File", file=f)
        print("---------------------------", file=f)
        print(file=f)
        print(station_links.comment1, file=f)
